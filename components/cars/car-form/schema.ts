...
const CarRentalPricingSchema = z.array(z.object({
    currency: z.nativeEnum(Currencies, {
        required_error: "Currency is required",
        invalid_type_error: `Currency must be ${Object.values(Currencies).join(" or ")}`
    }),
    unit: z.nativeEnum(CarPricingUnit, {
        required_error: "Unit is required",
        invalid_type_error: `Unit must be ${Object.values(CarPricingUnit).join(" or ")}`
    }),
    price: z.number({
    ...
    "mileageLimit": z.number({
        invalid_type_error: "Mileage Limit must be a number"
    }).optional().default(0)
})).optional().default([])
...
const CarMediaSchema = z.object({
    ...
    }).optional().default(MediaType.IMAGE),
    title: z.string({
...
const CarTranslationsSchema = z.array(z.object({
    locale: z.string({
    ...
    message: "Provide Translations for all languages"
})
...
const SecurityDepositSchema = z.object({
    currency: z.nativeEnum(Currencies, {
        message: "Invalid Security deposit currency",
    }),
    amount: z.number({
...
const CarDocumentsSchema = z.array(z.object({
    type: z.string({
...
const CarDimensionSchema = z.object({
    length: z.number({
...
const CarFormSchema = z.object({
    ...
    fuelTankSize: z.number({
        invalid_type_error: "fuel tank size must be a number",
    }).optional().default(0),
    ...
    engineSpecs: CarEngineSchema,
    dimensions: CarDimensionSchema,
    ...
    rentalPricings: CarRentalPricingSchema,
    securityDeposit: SecurityDepositSchema,
    documents: CarDocumentsSchema,
    owner: CarOwnerSchema,
    translations: CarTranslationsSchema
});
...
const mapper={
    ...
    year: car?.year ?? 0,
    category: car?.category?.split(" ")?.join("_") ?? "",
    ...
    listingType: car?.listingType?.toString()?.split(" ")?.join("_") ?? "",
    ...
    return mapper as unknown as z.infer<typeof CarFormSchema>;
}

export { CarFormSchema, CarCategorySchema, FuelTypeSchema, TransmissionTypeSchema, CarStatusSchema, CarConditionSchema, CarInspectionStatusSchema, CarMediaSchema, CarRentalPricingSchema, CarTranslationsSchema };
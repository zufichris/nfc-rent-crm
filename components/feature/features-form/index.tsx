...
import { CarFeatureFormSchema, getCarFeaturesFormDefaultValue } from "./schema"
import type { IFeature } from "@/types/feature"
import type { z } from "zod"
import { createFeature, updateFeature } from "@/lib/actions/feature"
...
export function FeatureForm({ existingFeature }: { existingFeature?: IFeature }) {
    const form = useForm<FeatureFormValues>({
        resolver: zodResolver(CarFeatureFormSchema),
        defaultValues: getCarFeaturesFormDefaultValue(existingFeature),
        mode: "onChange",
    })
    ...
    async function onSubmit(data: FeatureFormValues) {
        setIsSubmitting(true)
        try {
            if (existingFeature) {
                const res = await updateFeature(existingFeature.id, data)
                if (!res.success) {
                    throw new Error(res.message)
                }
                toast.success("Feature updated successfully")
                router.push(`/fleet-management/features/${existingFeature.id}`)
            } else {
                ...
                router.push(`/fleet-management/features/${res.data.id}`)
            }
        } catch (error: any) {
    ...
    const validateStep = async (step: number): Promise<boolean> => {
        ...
        switch (step) {
            case 0:
                isValid = await form.trigger(["category", "isHighlighted"])
                break
            case 1:
                            ...
                            <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto">
                                {isSubmitting ? (
                                    <>
                                        ...
                                        {existingFeature ? "Updating..." : "Creating..."}
                                    </>
                                ) : existingFeature ? (
                                    "Update Feature"
                                ) : (
